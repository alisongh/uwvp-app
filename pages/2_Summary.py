...
if "update_df" not in st.session_state:
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        ...
        "Department/Program/Unit by Question", 
        "Question Category"
        ])
    ...
    with tab3:
        ...
        if st.session_state.button_pivot_clicked:
            ...
            group_df = manipulateData.pivot_df(
                update_df, 
                option, 
                col_options)
            chart_prep_df = manipulateData.pivot_df(
                update_df, 
                option, 
                ["Agency", "Department/Program/Unit"])
    ...
    with tab4:
        ...
        if st.session_state.button_summary_clicked:
            df = manipulateData.get_question_df(
                data, 
                data["question_columns"], 
                tmp_df, 
                option)
            st.subheader("Summary of Responses")
            df["Grand Total"] = df.drop(
