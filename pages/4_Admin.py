...
question_category_df = pd.DataFrame(list(data["question_category"].keys())[:-1], columns=["Category"])
...
if "update_df" not in st.session_state:
    ...
    with col1:
        ...
        with st.expander("See Data"):
            st.data_editor(agency_df, 
                           width=550, 
                           height=550,
                           hide_index=True,
                           num_rows="dynamic")
        agency_option = st.selectbox(
            "Choose the agency you want to change: ",
            agency_list
        )
        ...
        if st.session_state.button_change_agency_clicked:
            ...
            file = open(r"data_update.json", "w+", encoding="cp866")
    ...
    with col2:
        ...
        with st.expander("See Data"):
            st.data_editor(question_category_df, 
                         width=550, 
                         height=550,
                         hide_index=True,
                         num_rows="dynamic")
    ...
    with col3:
        ...
        with st.expander("See Data"):
            st.data_editor(update_df[["Respondent ID","IP Address"]], 
                         width=550, 
                         height=550,
                         hide_index=True, 
                         num_rows="dynamic")
